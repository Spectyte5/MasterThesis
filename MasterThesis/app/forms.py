...
class MaterialForm(forms.Form):
    ...
    material_name = forms.CharField(max_length=50, initial='Aluminum', help_text='Name of the material (Optional)', required=False)

class WaveForm(forms.Form):
    type_of_wave = forms.ChoiceField(choices=WAVETYPE_CHOICES, initial=1, help_text='Type of the wave, [Shear, Lamb, Axial]')
    modes_symmetric = forms.IntegerField(initial=5, help_text='Number of symmetric modes to find')
    modes_antisymmetric = forms.IntegerField(initial=5, help_text='Number of antisymmetric modes to find')
    ...
    wavestructure_cols = forms.IntegerField(initial=2, required=False, 
    ...
    number_of_fd_points = forms.IntegerField(initial=100, required=False, help_text='Number of frequency x thickness points to find')
    phase_velocity_step = forms.IntegerField(initial=100, required=False, help_text='Step between phase velocity points checked')

class PlotForm(forms.Form):
    type_of_modes = forms.ChoiceField(choices=MODETYPE_CHOICES, initial=1, help_text='Which wave types should be plotted [both, symmetric, antisymmetric]')	
    type_of_plots = forms.MultipleChoiceField(choices=PLOTTYPE_CHOICES, initial=1, help_text='Which plots should be plotted [Phase, Group, Wavenumber, Wavestrucure]')
    show_cutoff_freq = forms.BooleanField(required=False, help_text='Display plate velocites: Longitudinal, Shear, Rayleigh of the material on the plot (Optional)')
    show_velocities = forms.BooleanField(required=False, help_text='Display cutoff frequencies on the plot (Optional)')
...
def validate_wave(form):
    ...
    structure_frequencies  = eval(form[1].cleaned_data["wavestructure_freq"]) if form[1].cleaned_data["wavestructure_freq"] else None
...
def get_input_from_form(form):
    ...
    modes_num = (form[1].cleaned_data["modes_symmetric"], form[1].cleaned_data["modes_antisymmetric"])
    ...
    plotter = Plot(wave, plotted_modes, *show_optional, path=settings.MEDIA_ROOT, **kwargs)
